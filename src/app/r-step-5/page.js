...
export default function FacilitiesDetail() {
  ...
  />
        ...
        <main className="pt-[120px] pb-12 flex justify-center relative z-10" style={{ top: '35px',paddingBottom:'120px' }}>
          <div className="w-full max-w-[650px] px-4" style={{maxWidth:'900px'}}>
...
<div className="grid grid-cols-3 gap-4">
  ...
  `}
>
  <img src={item.icon} alt={item.label} className="w-[40px] h-[30px]" />
  <span className={`text-sm mt-2 font-medium ${selectedFacilities.includes(item.label) ? 'text-[#0052CC]' : 'text-[rgba(113,117,128,1)]'}`} style={{ fontSize: '12px', fontWeight: '500' }}>
    {item.label}
  </span>
</button>
  ...
  <div className="grid grid-cols-3 gap-4" >
    {societyAmenities.map((item, index) => (
      ...
      >
        <img src={item.icon} alt={item.label} className="w-[40px] h-[30px]"  />
        <span className="text-sm mt-2 text-center" style={{ fontSize: '12px', fontWeight: '500'}}>{item.label}</span>
      </button>
    ))}
  </div>
</div>
